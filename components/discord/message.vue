<template>
	<div class="flex w-full flex-row gap-2 rounded-xl p-2 align-top hover:bg-base-200 sm:gap-4 sm:p-4">
		<div class="discord-message-avatar-wrapper">
			...
			<NuxtIcon v-else-if="name === 'baddie'" name="ph:smiley-angry-fill" class="discord-message-avatar baddie h-full w-full" />
			<NuxtIcon v-else name="ph:shooting-star-fill" class="h-full w-full text-info" />
		</div>
		<div class="flex-grow gap-2 max-sm:text-xs">
			<div class="mb-0.5 flex flex-row items-center">
				...
				<span v-if="profile.app" class="app-badge">
					<NuxtIcon v-if="profile.verified" name="ph:check-fat-fill" class="mr-0.5 h-2 w-2 sm:h-3 sm:w-3" />
					<span>APP</span>
				</span>
			</div>
			<div><slot /></div>
