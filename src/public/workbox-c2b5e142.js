define('./workbox-c2b5e142.js', ['exports'], function (t) {
	'use strict';
	try {
	...
	const a = t => new URL(String(t), location.href).href.replace(new RegExp('^' + location.origin), '');
	class c {
		constructor() {
			this.t = new Map();
		}
		...
		handleRequest({ request: t, event: e }) {
			...
			if ((!a && this.s && (a = this.s), a)) {
				...
				return r instanceof Promise && this.i && (r = r.catch(n => this.i.handle({ url: s, request: t, event: e }))), r;
			}
		}
	...
	class y {
		constructor(t, e, { onupgradeneeded: s, onversionchange: n } = {}) {
			(this.o = null), (this.h = t), (this.u = e), (this.l = s), (this.p = n || (() => this.close()));
		}
		get db() {
			return this.o;
		}
			...
			if (!this.o)
				return (
					(this.o = await new Promise((t, e) => {
						...
						(n.onerror = () => e(n.error)),
							(n.onupgradeneeded = t => {
								s ? (n.transaction.abort(), n.result.close()) : 'function' == typeof this.l && this.l(t);
							}),
							(n.onsuccess = () => {
								const e = n.result;
	...
	class q {
		...
		async setTimestamp(t, e) {
			const s = { url: (t = m(t)), timestamp: e, cacheName: this.m, id: this.v(t) };
		...
		async expireEntries(t, e) {
			const s = await this.o.transaction('cache-entries', 'readwrite', (s, n) => {
					...
					i.onsuccess = () => {
						...
						if (s) {
							const n = s.value;
							n.cacheName === this.m && ((t && n.timestamp < t) || (e && a >= e) ? r.push(s.value) : a++), s.continue();
						} else n(r);
					};
				}),
				n = [];
	...
	class v {
		constructor(t, e = {}) {
			(this.R = !1), (this.U = !1), (this.L = e.maxEntries), (this.N = e.maxAgeSeconds), (this.m = t), (this._ = new q(t));
		}
		async expireEntries() {
	...
	const R = (t, e) => t.filter(t => e in t),
		U = async ({ request: t, mode: e, plugins: s = [] }) => {
			...
			let i = t;
			for (const t of n)
				(i = await t.cacheKeyWillBeUsed.call(t, { mode: e, request: i })), 'string' == typeof i && (i = new Request(i));
			return i;
		},
		L = async ({ cacheName: t, request: e, event: s, matchOptions: n, plugins: i = [] }) => {
			...
			for (const e of i)
				if ('cachedResponseWillBeUsed' in e) {
					...
					c = await i.call(e, { cacheName: t, event: s, matchOptions: n, cachedResponse: c, request: a });
				}
			return c;
		},
		x = async ({ cacheName: t, request: e, response: n, event: i, plugins: r = [], matchOptions: c }) => {
			...
			for (const e of l) await e.cacheDidUpdate.call(e, { cacheName: t, event: i, oldResponse: f, newResponse: h, request: o });
		},
		N = L,
		b = async ({ request: t, fetchOptions: e, event: n, plugins: i = [] }) => {
			if (('string' == typeof t && (t = new Request(t)), n instanceof FetchEvent && n.preloadResponse)) {
			...
			try {
				...
				for (const t of i) 'fetchDidSucceed' in t && (s = await t.fetchDidSucceed.call(t, { event: n, request: c, response: s }));
				return s;
			} catch (t) {
				for (const e of r) await e.fetchDidFail.call(e, { error: t, event: n, originalRequest: a.clone(), request: c.clone() });
				throw t;
			}
		};
	try {
	...
	const O = { cacheWillUpdate: async ({ response: t }) => (200 === t.status || 0 === t.status ? t : null) };
	let E;
		...
		addToCacheList(t) {
			...
			for (const n of t) {
				...
				if (this.O.has(i) && this.O.get(i) !== t)
					throw new s('add-to-cache-list-conflicting-entries', { firstEntry: this.O.get(i), secondEntry: t });
		...
		async install({ event: t, plugins: e } = {}) {
			...
			const c = s.map(({ cacheKey: s, url: n }) => {
				const i = this.M.get(s),
					r = this.K.get(n);
				return this.P({ cacheKey: s, cacheMode: r, event: t, integrity: i, plugins: e, url: n });
			});
			await Promise.all(c);
		...
		createHandler(t = !0) {
			return async ({ request: e }) => {
				try {
					...
					if (t) return t;
					throw new s('missing-precache-entry', { cacheName: this.m, url: e instanceof Request ? e.url : e });
				} catch (s) {
					if (t) return fetch(e);
					throw s;
				}
	...
	const k = (t, e) => {
		...
		for (const n of (function* (t, { ignoreURLParametersMatching: e, directoryIndex: s, cleanURLs: n, urlManipulation: i } = {}) {
		...
		C ||
			((({
				...
				self.addEventListener('fetch', r => {
					const a = k(r.request.url, { cleanURLs: s, directoryIndex: e, ignoreURLParametersMatching: t, urlManipulation: n });
					if (!a) return;
					let c = self.caches
						.open(i)
						.then(t => t.match(a))
	...
	(t.CacheFirst = class {
		constructor(t = {}) {
			(this.m = w(t.cacheName)), (this.T = t.plugins || []), (this.D = t.fetchOptions), (this.k = t.matchOptions);
		}
		async handle({ event: t, request: e }) {
			'string' == typeof e && (e = new Request(e));
			let n,
				i = await N({ cacheName: this.m, request: e, event: t, matchOptions: this.k, plugins: this.T });
			if (!i)
				try {
					i = await this.C(e, t);
				} catch (t) {
		...
		async C(t, e) {
			const s = await b({ request: t, event: e, fetchOptions: this.D, plugins: this.T }),
				n = s.clone(),
				i = x({ cacheName: this.m, request: t, response: n, event: e, plugins: this.T });
			if (e)
				try {
					e.waitUntil(i);
				} catch (t) {}
		...
		(t.NetworkFirst = class {
			constructor(t = {}) {
				...
				(this.B = t.networkTimeoutSeconds || 0), (this.D = t.fetchOptions), (this.k = t.matchOptions);
			}
			async handle({ event: t, request: e }) {
				const n = [];
				'string' == typeof e && (e = new Request(e));
			...
			async j({ timeoutId: t, request: e, logs: s, event: n }) {
				let i, r;
				try {
					r = await b({ request: e, event: n, fetchOptions: this.D, plugins: this.T });
				} catch (t) {
					i = t;
				}
				if ((t && clearTimeout(t), i || !r)) r = await this.G({ request: e, event: n });
					...
					const t = r.clone(),
						s = x({ cacheName: this.m, request: e, response: t, event: n, plugins: this.T });
					if (n)
						try {
							n.waitUntil(s);
						} catch (t) {}
			...
			G({ event: t, request: e }) {
				return N({ cacheName: this.m, request: e, event: t, matchOptions: this.k, plugins: this.T });
			}
		}),
		(t.StaleWhileRevalidate = class {
			...
			async handle({ event: t, request: e }) {
				...
				let i,
					r = await N({ cacheName: this.m, request: e, event: t, matchOptions: this.k, plugins: this.T });
				if (r) {
					if (t)
						try {
							t.waitUntil(n);
			...
			async C({ request: t, event: e }) {
				const s = await b({ request: t, event: e, fetchOptions: this.D, plugins: this.T }),
					n = x({ cacheName: this.m, request: t, response: s.clone(), event: e, plugins: this.T });
				if (e)
					try {
						e.waitUntil(n);
					} catch (t) {}
		...
		(t.precacheAndRoute = function (t, e) {
			!(function (t) {
				D().addToCacheList(t), t.length > 0 && (self.addEventListener('install', I), self.addEventListener('activate', W));
			})(t),
				A(e);
		}),
